...
class Stamp implements Drawable {
  ...
  draw(ctx: CanvasRenderingContext2D) {
    ...
    ctx.fillText(
      this.stampString,
      this.position.x - this.sizeScalar / 2,
      this.position.y + this.sizeScalar / 2
    );
  }
  drag(x: number, y: number) {
    this.position = { x, y };
...
canvas.addEventListener("tool-moved", (_e) => {
...
canvas.addEventListener("mousedown", (_e) => {
  ...
  if (currTool) {
    if (currTool.type === "line") {
      ...
      currDrawableBuffer = new Stamp(currTool.size, currTool.symbol!, {
        x: currMouseX,
        y: currMouseY,
      });
    }
...
// UNDO BUTTON
const undoButton = createButtonWithText("Undo", () => {
...
// REDO BUTTON
const redoButton = createButtonWithText("Redo", () => {
  // When redo button clicked
  drawManager.redo();
  canvas.dispatchEvent(new Event("drawing-changed"));
});
...
const fnSetSize = function (size: number) {
  // function to thicken the marker
...
const exportButton = createButtonWithText("Export", () => {
  const exportCanvas = addCanvas(
    CANVAS_WIDTH * SAVE_SCALE,
    CANVAS_HEIGHT * SAVE_SCALE
  );
